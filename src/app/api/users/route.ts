...
export const POST = async (request: NextRequest) => {
    try {
        ...
        switch (action) {
            case "getUsers":
                response = await getUsers(requestBody, authUser);
                break;

            case "addUser":
                response = await addUser(requestBody, authUser);
                break;

            case "updateUser":
                response = await updateUser(requestBody, authUser);
                break;

            case "toggleUserStatus":
                response = await toggleUserStatus(requestBody, authUser);
                break;

            case "getAssignableUsers":
                response = await getAssignableUsers(requestBody, authUser);
                break;
