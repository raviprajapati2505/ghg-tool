...
import BusinessTravelModal from "../projectFormModal/businessTravelModal";
...
import { formatDateDisplay, formatDateTime } from "@/utils/function";

export default function BusinessTravel({ pid, scid }: { pid: string; scid: string }) {
    ...
    const handleDownloadTemplate = () => {
        const templateUrl = getTemplateUrl('business-travel');
        if (templateUrl) {
            const link = document.createElement('a');
            link.href = templateUrl;
            link.download = 'Business_Travel_Template.xlsx';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const expectedFields = getTemplateFields('business-travel');
            const columnMapping = getColumnMapping('business-travel');
    ...
    return (
        <div className="bg-white rounded-xl shadow p-3">
            <div className="d-flex justify-content-between">
                <p className="main-title mb-3">Business Travel
                </p>
                ...
                <table className="w-100 bg-white">
                    <thead className="text-white bg-gray-800 z-10">
                        <tr>
                            ...
                            <th className="text-start">Flight date</th>
                            <th className="text-start">Flight Type</th>
                            <th className="text-start">Flight Class</th>
                            <th className="text-start">From</th>
                            <th className="text-start">Destinations</th>
                            <th className="text-start">Status</th>
                            <th className="text-start" style={{ width: "200px" }}>Action</th>
                        </tr>
                    </thead>
                        ...
                        {Array.isArray(currentPageData) &&
                            ...
                            currentPageData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="text-start">{formatDateDisplay(item?.flight_date)}</td>
                                    <td className="text-start">{item.flight_type}</td>
                                    <td className="text-start">{item.flight_class}</td>
                                    <td className="text-start">{item.from}</td>
                                    <td className="text-start">{Array.isArray(item?.destinations) && item?.destinations.length > 0 ? item.destinations.join("/") : ""}</td>
