...
import EmployeeCommuteModal from "../projectFormModal/employeeCommuteModal";
...
export default function EmployeeCommute({ pid, scid }: { pid: string; scid: string }) {
    ...
    const handleDownloadTemplate = () => {
        const templateUrl = getTemplateUrl('employee-commute');
        if (templateUrl) {
            const link = document.createElement('a');
            link.href = templateUrl;
            link.download = 'Employee_Commute_Template.xlsx';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const expectedFields = getTemplateFields('employee-commute');
            const columnMapping = getColumnMapping('employee-commute');
    ...
    return (
        <div className="bg-white rounded-xl shadow p-3">
            <div className="d-flex justify-content-between">
                <p className="main-title mb-3">Employee Commute</p>
                ...
                <table className="w-100 bg-white">
                    <thead className="text-white bg-gray-800 z-10">
                        <tr>
                            ...
                            <th className="text-start">Working Days</th>
                            <th className="text-start">Employees</th>
                            <th className="text-start">Car Commuters</th>
                            <th className="text-start">Petrol Users</th>
                            <th className="text-start">Petrol Avg (KM)</th>
                            <th className="text-start">Diesel Users</th>
                            <th className="text-start">Diesel Avg (KM)</th>
                            <th className="text-start">Hybrid Users</th>
                            <th className="text-start">Hybrid Avg (KM)</th>
                            <th className="text-start">EV Users</th>
                            <th className="text-start">EV Avg (KM)</th>
                            <th className="text-start">Metro Commuters</th>
                            <th className="text-start">Metro Avg (KM)</th>
                            <th className="text-start">Walking Commuters</th>
                            <th className="text-start">Walking Avg (KM)</th>
                        ...
                        {Array.isArray(currentPageData) &&
                            ...
                            currentPageData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="text-start">{item.working_days}</td>
                                    <td className="text-start">{item.employees}</td>
                                    <td className="text-start">{item.car_commuters}</td>
                                    <td className="text-start">{item.petrol_users}</td>
                                    <td className="text-start">{item.petrol_avg_km}</td>
                                    <td className="text-start">{item.diesel_users}</td>
                                    <td className="text-start">{item.diesel_avg_km}</td>
                                    <td className="text-start">{item.hybrid_users}</td>
                                    <td className="text-start">{item.hybrid_avg_km}</td>
                                    <td className="text-start">{item.ev_users}</td>
                                    <td className="text-start">{item.ev_avg_km}</td>
                                    <td className="text-start">{item.metro_commuters}</td>
                                    <td className="text-start">{item.metro_avg_km}</td>
                                    <td className="text-start">{item.walking_commuters}</td>
                                    <td className="text-start">{item.walking_avg_km}</td>
