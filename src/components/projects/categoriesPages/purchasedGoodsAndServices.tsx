...
import PurchasedGoodsAndServicesModal from "../projectFormModal/purchasedGoodsAndServicesModal";
...
export default function PurchasedGoodsAndServices({ pid, scid }: { pid: string; scid: string }) {
    ...
    const handleDownloadTemplate = () => {
        const templateUrl = getTemplateUrl('purchased-goods-and-services');
        if (templateUrl) {
            const link = document.createElement('a');
            link.href = templateUrl;
            link.download = 'Purchased_Goods_Services_Template.xlsx';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const expectedFields = getTemplateFields('purchased-goods-and-services');
            const columnMapping = getColumnMapping('purchased-goods-and-services');
    ...
    } = usePagination(categoryDataList);
    ...
    return (
        <div className="bg-white rounded-xl shadow p-3">
            <div className="d-flex justify-content-between">
                <p className="main-title mb-3">Purchased Goods and Services</p>
            ...
            <div className="table-responsive-div">
                <table className="w-100 bg-white">
                    <thead className="text-white bg-gray-800 z-10">
                        <tr>
                            <th>#</th>
                            <th className="text-start">Date of Expense</th>
                            <th className="text-start">Description of Purchase / Expense</th>
                            <th className="text-start">Currency</th>
                            <th className="text-start">Amount Debited</th>
                        ...
                        {Array.isArray(currentPageData) &&
                            ...
                            currentPageData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="text-start"> {formatDateDisplay(item?.date_of_expense)}</td>
                                    <td className="text-start">{item?.description}</td>
                                    <td className="text-start">{item?.currency}</td>
            ...
            />
            <PurchasedGoodsAndServicesModal
                isOpen={isModalOpen}
                onClose={() => { closeModal() }}
                index={selectedRowIndex}
