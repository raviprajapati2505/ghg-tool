...
import FugitiveEmissionsModal from "../projectFormModal/fugitiveEmissionsModal";
...
export default function FugitiveEmissions({ pid, scid }: { pid: string; scid: string }) {
    ...
    const handleDownloadTemplate = () => {
        const templateUrl = getTemplateUrl('fugitive-emissions');
        if (templateUrl) {
            const link = document.createElement('a');
            link.href = templateUrl;
            link.download = 'Fugitive_Emissions_Template.xlsx';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const expectedFields = getTemplateFields('fugitive-emissions');
            const columnMapping = getColumnMapping('fugitive-emissions');
    ...
    return (
        <div className="bg-white rounded-xl shadow p-3">
            <div className="d-flex justify-content-between">
                <p className="main-title mb-3">Fugitive Emissions</p>
                ...
                <table className="w-100 bg-white">
                    <thead className="text-white bg-gray-800 z-10">
                        <tr>
                            ...
                            <th className="text-start">Equipment ID</th>
                            <th className="text-start">Equipment Type</th>
                            <th className="text-start">Refrigerant Used</th>
                            <th className="text-start">Total Consumption (Refill) in Reporting period</th>
                        ...
                        {Array.isArray(currentPageData) &&
                            ...
                            currentPageData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="text-start">{item?.equipment_id}</td>
                                    <td className="text-start">{item?.equipment_type}</td>
                                    <td className="text-start">{item?.refrigerant_used}</td>
                                    <td className="text-start">{item?.total_consumption_refill}</td>
