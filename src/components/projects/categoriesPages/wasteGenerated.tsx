...
import WasteGeneratedModal from "../projectFormModal/wasteGeneratedModal";
...
export default function WasteGenerated({ pid, scid }: { pid: string; scid: string }) {
    ...
    const handleDownloadTemplate = () => {
        const templateUrl = getTemplateUrl('waste-generated');
        if (templateUrl) {
            const link = document.createElement('a');
            link.href = templateUrl;
            link.download = 'Waste_Generated_Template.xlsx';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const expectedFields = getTemplateFields('waste-generated');
            const columnMapping = getColumnMapping('waste-generated');
            if (!expectedFields) {
    ...
    return (
        <div className="bg-white rounded-xl shadow p-3">
            <div className="d-flex justify-content-between">
                <p className="main-title mb-3">Waste Generated</p>
                ...
                <table className="w-100 bg-white">
                    <thead className="text-white bg-gray-800 z-10">
                        <tr>
                            ...
                            <th className="text-start">Total Waste Collected</th>
                            <th className="text-start">General Waste</th>
                            <th className="text-start">Waste Type</th>
                            <th className="text-start">Treatment type</th>
                            <th className="text-start">Recyclable Waste</th>
                            <th className="text-start">Paper Waste</th>
                            <th className="text-start">Plastic Waste</th>
                        ...
                        {Array.isArray(currentPageData) &&
                            ...
                            currentPageData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="text-start">{item?.total_waste_collected}</td>
                                    <td className="text-start">{item?.general_waste}</td>
                                    <td className="text-start">{item?.waste_type}</td>
                                    <td className="text-start">{item?.treatment_type}</td>
                                    <td className="text-start">{item?.recyclable_waste}</td>
                                    <td className="text-start">{item?.paper_waste}</td>
                                    <td className="text-start">{item?.plastic_waste}</td>
