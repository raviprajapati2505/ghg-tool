...
import StationaryCombustionModal from "../projectFormModal/stationaryCombustionModal";
...
export default function StationaryCombustion({ pid, scid }: { pid: string; scid: string }) {
    ...
    const handleDownloadTemplate = () => {
        const templateUrl = getTemplateUrl('stationary-combustion');
        if (templateUrl) {
            const link = document.createElement('a');
            link.href = templateUrl;
            link.download = 'Stationary_Combustion_Template.xlsx';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const expectedFields = getTemplateFields('stationary-combustion');
            const columnMapping = getColumnMapping('stationary-combustion');
    ...
    return (
        <div className="bg-white rounded-xl shadow p-3">
            <div className="d-flex justify-content-between">
                <p className="main-title mb-3">Stationary Combustion</p>
                ...
                <table className="w-100 bg-white">
                    <thead className="text-white bg-gray-800 z-10">
                        <tr>
                            ...
                            <th className="text-start">Asset / Equipment ID</th>
                            <th className="text-start">Source of Combustion</th>
                            <th className="text-start">Type of fuel used</th>
                            <th className="text-start" title="Total Fuel Consumption during reporting period">Total Fuel Consumption</th>
                            <th className="text-start">Consumption Unit</th>
                        ...
                        {Array.isArray(currentPageData) &&
                            ...
                            currentPageData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="text-start">{item?.asset_id}</td>
                                    <td className="text-start"> {item?.source_of_combustion}</td>
                                    <td className="text-start">{item?.fuel_type}</td>
                                    <td className="text-start">{item?.total_consumption}</td>
                                    <td className="text-start"> {item?.consumption_unit}</td>
