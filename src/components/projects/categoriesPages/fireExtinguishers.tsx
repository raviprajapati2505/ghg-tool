...
import FireExtinguishersModal from "../projectFormModal/fireExtinguishersModal";
...
export default function FireExtinguishers({ pid, scid }: { pid: string; scid: string }) {
    ...
    const handleDownloadTemplate = () => {
        const templateUrl = getTemplateUrl('fire-extinguishers');
        if (templateUrl) {
            const link = document.createElement('a');
            link.href = templateUrl;
            link.download = 'Fire_Extinguishers_Template.xlsx';
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const expectedFields = getTemplateFields('fire-extinguishers');
            const columnMapping = getColumnMapping('fire-extinguishers');
    ...
    return (
        <div className="bg-white rounded-xl shadow p-3">
            <div className="d-flex justify-content-between">
                <p className="main-title mb-3">Fire Extinguishers</p>
                ...
                <table className="w-100 bg-white">
                    <thead className="text-white bg-gray-800 z-10">
                        <tr>
                            ...
                            <th className="text-start">Office</th>
                            <th className="text-start">Location</th>
                            <th className="text-start">Office ID</th>
                            <th className="text-start"
                                title="Total Quantity of CO2 Refilled during Reporting period">
                                Total Quantity of CO2
                            </th>
                            <th className="text-start">Consumption Units</th>
                        ...
                        {Array.isArray(currentPageData) &&
                            ...
                            currentPageData.map((item, index) => (
                                ...
                                >
                                    ...
                                    <td className="text-start">{item?.office}</td>
                                    <td className="text-start">{item?.location}</td>
                                    <td className="text-start">{item?.office_id}</td>
                                    <td className="text-start">{item?.total_quantity_of_co2}</td>
                                    <td className="text-start">{item?.consumption_units}</td>
